...
CONFIG = {
    ...
    'SYMBOLS': {
        'AAVE/USDT':   {'exchange': 'okx', 'scalp': True, 'pulse': False},
        'ADA/USDT':    {'exchange': 'okx', 'scalp': True, 'pulse': False},
        'APT/USDT':    {'exchange': 'okx', 'scalp': True, 'pulse': True},
        'ARB/USDT':    {'exchange': 'okx', 'scalp': True, 'pulse': True},
        'AVAX/USDT':   {'exchange': 'okx', 'scalp': True, 'pulse': False},
        ...
        'HYPE/USDT':   {'exchange': 'okx', 'scalp': True, 'pulse': False, 'okx_inst_id': 'HYPE-USDT-SWAP'},
        'INJ/USDT':    {'exchange': 'okx', 'scalp': True, 'pulse': True},
        'LDO/USDT':    {'exchange': 'okx', 'scalp': False, 'pulse': False},
        'LINK/USDT':   {'exchange': 'okx', 'scalp': True, 'pulse': False},
        ...
        'LTC/USDT':    {'exchange': 'okx', 'scalp': True, 'pulse': False},
        'NEAR/USDT':   {'exchange': 'okx', 'scalp': True, 'pulse': False},
        ...
        'SOL/USDT':    {'exchange': 'okx', 'scalp': True, 'pulse': False},
        'STX/USDT':    {'exchange': 'okx', 'scalp': False, 'pulse': False},
        'SUI/USDT':    {'exchange': 'okx', 'scalp': True, 'pulse': False},
        'TAO/USDT':    {'exchange': 'okx', 'scalp': True, 'pulse': False},  # perp-only
        'TIA/USDT':    {'exchange': 'okx', 'scalp': False, 'pulse': False},
        'UNI/USDT':    {'exchange': 'okx', 'scalp': True, 'pulse': False},
        ...
        'XRP/USDT':    {'exchange': 'okx', 'scalp': True, 'pulse': False},
        'ZEC/USDT':    {'exchange': 'okx', 'scalp': True, 'pulse': False},
        'ZEN/USDT':    {'exchange': 'okx', 'scalp': True, 'pulse': True},
    },
...
def track_alert(symbol, strategy):
    if symbol not in WEEKLY_STATS:
        WEEKLY_STATS[symbol] = {'DAILY': 0, 'PULSEV5': 0}
...
def is_pulse_enabled():
    return bool(CONFIG.get('ENABLE_PULSE_V5', True))
...
def get_market_context_info() -> str:
    ...
    def ctx_str(symbol):
        ctx_2h = MOMENTUM_STATE.get(symbol, {}).get('st_context_2h')
        ctx_12h = MOMENTUM_STATE.get(symbol, {}).get('st_context_12h')
        parts = []
        if ctx_2h: parts.append(f"2H:{ctx_2h.upper()}")
        if ctx_12h: parts.append(f"12H:{ctx_12h.upper()}")
...
def send_weekly_report():
    ...
    total_pulse_v5   = sum(s.get('PULSEV5', 0)      for s in WEEKLY_STATS.values())

    msg += (
        "📋 <b>Par stratégie:</b>\n"
        f"  — DAILY: {total_daily}\n"
        f"  — PULSEV5: {total_pulse_v5}\n\n"
    )
    ...
    if assets_with_alerts:
        ...
        for symbol, stats in sorted(assets_with_alerts.items(), key=lambda x: sum(x[1].values()), reverse=True):
            ...
            if stats.get('PULSEV5', 0):     details.append(f"PL5:{stats['PULSEV5']}")
...
def init_symbol_states(symbol):
    if symbol not in MOMENTUM_STATE:
        MOMENTUM_STATE[symbol] = {
            'st_context_15m': None, 'st_context_30m': None,
            'st_context_15m_ts': None, 'st_context_30m_ts': None,
    ...
    try:
        ...
        if alert_type == 'st_context':
            ...
            if tf == '1m':
                ...
                check_context2h_rci2h_ctx10m_info(symbol, price=price)
            elif tf == '2d':
        ...
        if is_pulse_enabled() and is_pulse_symbol(symbol) and (
            ...
            pulse_trigger_dir = None
            if alert_type == 'zalt' and tf == '10m' and zalt_signal in ('trend_flip', 'flip'):
                pulse_trigger_dir = parsed_zalt
            evaluate_pulse_v3(
                symbol,
...
def sync_scalp():
    """Rechauffe le scalpbot : Bias 30m/2H + RCI 30m + ST Context 1m/10m/30m + ZALT 10m/30m.
    ...
    for symbol in sorted(scalp_symbols):
        ...
        if zalt10 in ('buy', 'sell'):
            try:
                ...
                resp = requests.post(f"{scalp_url}/webhook", json=payload, timeout=5)
                if resp.status_code == 200:
                    symbol_sent.append('zalt10m')
                else:
                    errors.append(f"{symbol}: ZALT10M HTTP {resp.status_code}")
            except Exception as e:
                errors.append(f"{symbol}: ZALT10M {e}")
        else:
...
def fetch_ohlcv_okx(symbol, timeframe, limit=250):
    ...
    try:
        ...
        tf_map = {'5m': '5m', '10m': '10m', '15m': '15m', '30m': '30m', '1h': '1H', '2h': '2H', '3h': '3H', '4h': '4H', '6h': '6H', '12h': '12H', '1d': '1D'}
...
def update_okx_rci_30m(symbol):
    ...
    evaluate_pulse_v3(
        ...
        price=price or 0.0,
        exchange_name=get_symbol_config(symbol).get('exchange', 'okx'),
        event_id=f"okx_rci_30m_{symbol}_{int(now_ts)}",
        source='okx_rci_30m',
    )
...
def update_okx_rci_2h(symbol):
    ...
    with STATE_LOCK:
        init_symbol_states(symbol)
        m = MOMENTUM_STATE[symbol]
        m['rci_2h_10'] = rci_values.get(10)
        m['rci_2h_30'] = rci_values.get(30)
        m['rci_2h_50'] = rci_values.get(50)
...
def update_okx_rci_4h(symbol):
    """RCI 10/30/50 sur bougies 4H confirmees (OKX), tendance Pulse V5."""
...
def check_pulse_v4_prep(symbol, price=0.0, source='state_refresh'):
    """Pulse V5 n'a pas d'alerte PREP separee."""
    return False

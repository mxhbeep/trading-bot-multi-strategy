import os, time, json, requests, logging, threading, redis
from flask import Flask, request, jsonify
from datetime import datetime, timezone

# --- CONFIGURATION ---
TOKEN = os.environ.get('TELEGRAM_BOT_TOKEN', '8110041550:AAHJKAWxIG1ZBjZ8fRfFMKq-4iTeo5v4-Hw')
CHAT_ID = os.environ.get('TELEGRAM_CHAT_ID', '6473214015')
PORT = int(os.environ.get("PORT", 8080))
MIN_INTERVAL = 1800  # Cooldown 30min pour éviter le spam
...
def audit_log(data, status="reçu"):
    ...
    try:
        ...
        r.ltrim('audit_trail', 0, 499) # Garde les 500 derniers
    except: pass
...
def webhook():
    ...
    if r:
        ...
        if "ema" in alert_type and tf in ["1h", "60"]:
            # On prend la valeur peu importe le nom du champ envoyé par TV
            ema_val = data.get('value', data.get('ema200', data.get('indicator_value')))
            if ema_val:
                r.hset(f"ctx:{symbol}", "ema", float(ema_val))
                logger.info(f"EMA {symbol} -> {ema_val}")

        # Signal d'Achat (Supertrend 1H)
        if alert_type == "supertrend" and tf in ["1h", "60"] and val == "buy":
            ...
            if macd_ok and ema_ok:
                ...
                if time.time() - last_buy > MIN_INTERVAL:
                    ...
                    logger.info(f"Cooldown pour {symbol}")
